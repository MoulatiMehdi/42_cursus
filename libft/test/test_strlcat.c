...
/*   Updated: 2024/10/25 16:46:44 by mmoulati         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_response	is_test_passed(t_strstr_request *req)
{
	...
	for (i = 0; i < 2 * len; i++)
	{
		expected = strlcat(req->s1, req->s2, i);
		result = ft_strlcat(req->s1, req->s2, i);
...
void	test_strlcat(void)
{
	...
	run_test("ft_strlcat", g_strstr_cases, is_test_passed, strstr_size
