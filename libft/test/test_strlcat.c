...
/*   Updated: 2024/10/25 21:54:51 by mmoulati         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_response	is_test_passed(t_strstr_request *req)
{
	...
	for (i = 0; i < 5 * len; i++)
	{
		...
		expected = strlcat(exp_dst, req->s1, i);
		result = ft_strlcat(res_dst, req->s1, i);
		...
		if (!res.is_pass)
		{
			sprintf(res.result, "res_str : \x1b[31m'%s'\x1b[0m when i = %3lu",
				res_dst, i);
			sprintf(res.expected, "exp_str : \x1b[32m'%s'\x1b[0m when i = %3lu",
				exp_dst, i);
			break ;
		}
	}
...
void	test_strlcat(void)
{
	...
	run_test("ft_strlcat", g_strl_cases, is_test_passed, strl_size / strl_elem,
		strl_elem);
}
